...
func TestEncodeDecodeText(t *testing.T) {
	for _, tc := range testCasesEncoder {
		encoded, err := TextEncode(tc.input, tc.encoding)
		if err != nil {
			t.Errorf("Encoding failed for %s: %v", tc.encoding, err)
		}
		decoded, err := TextDecode(encoded, tc.encoding, TextDecoderOptions{Fatal: true, IgnoreBOM: true}, TextDecodeOptions{Stream: true})
		if err != nil {
			t.Errorf("Decoding failed for %s: %v", tc.encoding, err)
		}
		if decoded != tc.input {
			t.Errorf("Mismatch for %s: expected %q, got %q", tc.encoding, tc.input, decoded)
		}
	}
}
